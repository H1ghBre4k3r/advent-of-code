...
const INPUT: &str = "turn on 489,959 through 759,964
...
type SubGrid = ((usize, usize), (usize, usize));
...
fn parse_instruction(instruction: &str) -> Action {
    ...
    match action {
        "toggle" => Action::Toggle(parse_sub_grid(&mut words)),
        "turn" => {
            let mode = words.next().unwrap();
            match mode {
                "on" => Action::On(parse_sub_grid(&mut words)),
                "off" => Action::Off(parse_sub_grid(&mut words)),
                _ => unreachable!(),
            }
        }
        _ => unreachable!(),
...
fn parse_sub_grid(words: &mut SplitWhitespace) -> SubGrid {
...
fn solution_1() {
    ...
    while let Some(instruction) = instructions.next() {
        match parse_instruction(instruction) {
            Action::Off(sub_grid) => {
                for x in sub_grid.0 .0..=sub_grid.1 .0 {
                    for y in sub_grid.0 .1..=sub_grid.1 .1 {
                        grid[x][y] = false;
                    }
                }
            }
